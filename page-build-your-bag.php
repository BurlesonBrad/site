					...
					function getDiscBags() {
						...
						if ( isset($byb) ) {
							...
							foreach ( $bags as $bag ):
								...
								function the_disc($d) {
									$slug = $d["slug"];
									echo '<div class="disc product ' . get_disc( $slug, 'slug' ) . '" data-product-slug="' . get_disc( $slug, 'slug' ) . '"><a href="' . get_disc( $slug, 'link' ) . '">' . get_disc( $slug, 'thumbnail' ) . '</a><img class="remove-from-bag" src="' . get_stylesheet_directory_uri() . '/images/trash-icon.png" alt="remove" /></div>';
								}
